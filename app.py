...
def generate():
    ...
    def stream():
        ...
        def evt(kind, **payload):
            return f"data: {json.dumps({'event': kind, **payload})}\n\n"

        try:
            yield evt("step", step=0, message="Researching pain points...")
            pains = pipeline.brainstorm_pains(keyword)

            yield evt("step", step=1, message="Generating solutions...")
            pipeline.generate_solutions(keyword, pains)

            yield evt("step", step=2, message="Crafting $100M offer...")
            offer = pipeline.create_offer(keyword, pains)

            yield evt("step", step=3, message="Creating ebook title...")
            title_data = pipeline.create_title(offer)
            title = title_data["selectedTitle"]

            yield evt("step", step=4, message=f'Building outline for "{title}"...')
            outline = pipeline.build_outline(title, keyword)

            yield evt("step", step=5, message="Generating cover image...")
            cover_path = pipeline.generate_cover(title, keyword, session_id, OUTPUT_DIR)

            yield evt("step", step=6, message="Writing full ebook...")
            ebook_content = pipeline.write_ebook(title, keyword, outline)

            yield evt("step", step=7, message="Creating value enhancer package...")
            value_data = pipeline.create_value_enhancer(title, keyword, offer)

            yield evt("step", step=8, message="Building ebook DOCX...")
            ebook_file = f"ebook_{session_id}.docx"
            build_ebook_docx(
                title,
                ebook_content,
                cover_path,
                os.path.join(OUTPUT_DIR, ebook_file),
            )

            yield evt("step", step=9, message="Building value enhancer DOCX...")
            value_file = f"value_enhancer_{session_id}.docx"
            build_value_enhancer_docx(
                value_data, os.path.join(OUTPUT_DIR, value_file)
            )
